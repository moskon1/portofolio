...
function matchable(value:string) {
...
export default async function handler(req:ApiRequest,res:ApiResponse) {
  ...
  try {
    ...
    const prompt=`Create a Romanian hospitality sales-demo JSON from this TuristInfo text. ${blueprint} Extract facts first. Write concise Romanian copy only. Mark assumptions groq-mock. For reviews, copy at most 3 review comments verbatim from REVIEWS_TEXT; reviewer identity will be replaced by the application. Never invent or paraphrase review comments. Use the real starting price as the lowest room price. Rivers, forests, mountains, lakes and landmarks belong only in attractions, never in facilities or room amenities. Never invent attraction distances; use "in apropiere" when the source gives no distance. Return exactly one JSON object with no markdown.\n\nSOURCE:${source.href}\nLISTING:${listingText}\nREVIEWS:${reviewsText}`;
    ...
    const reviewProfiles=[
      {author:'Elena & Andrei M.',location:'Bucure\u0219ti, Rom\u00e2nia',date:'Iulie 2026',avatar:'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=200&q=80'},
      {author:'Marcus Vance',location:'M\u00fcnchen, Germania',date:'Iunie 2026',avatar:'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&q=80'},
      {author:'Cristina & Radu D.',location:'Cluj-Napoca, Rom\u00e2nia',date:'August 2026',avatar:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=200&q=80'},
    ];
    const reviewCorpus=matchable(`${reviewsText} ${listingText}`);
    const reviews=(generated.reviews||[]).map((review:any)=>plainValue(review.comment)).filter((comment:string)=>comment.length>=24&&reviewCorpus.includes(matchable(comment).slice(0,32))).slice(0,3).map((comment:string,index:number)=>({...reviewProfiles[index],rating:5,comment,source:'turistinfo' as const,identitySource:'groq-mock' as const}));
    const demo={schemaVersion:1,slug:slugify(plainValue(generated.slug)||property.name||'hospitality-demo'),sourceUrl:source.href,generatedAt:new Date().toISOString(),status:'draft',property,images,heroImage:images[0]||'',facilities,rooms,attractions,reviews,provenance:generated.provenance||{}};
