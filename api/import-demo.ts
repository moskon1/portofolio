...
export default async function handler(req:ApiRequest,res:ApiResponse) {
  ...
  try {
    ...
    const blueprint=`Required shape: {slug,property:{name,type,address,cityRegion,phone,whatsapp,email,startingPriceRON,rating,reviewCount,heroTitle,shortDescription,fullDescription},facilities:string[],rooms:[{id,title,description,imageIndexes:number[],priceRON,capacityAdults,capacityKids,sizeSqm,amenities:string[],source}],attractions:[{title,description,distance,imageIndex,source}],reviews:[{author,location,date,rating,comment,source:"turistinfo"}],provenance:{property,facilities,rooms,attractions,reviews}}. Every title/description is {ro,en,de,no}. Every provenance/source value is one of turistinfo, description, groq-rewritten, groq-mock, manual.`;
    const prompt=`Create a personalized hospitality SALES DEMO JSON from the supplied TuristInfo data. ${blueprint} Extract facts first. Rewrite concise marketing content in Romanian, English, German and Norwegian. Fill missing presentation fields with plausible mock data marked groq-mock. Never fabricate reviews: include only reviews present in REVIEWS_TEXT, unchanged and at most 4. Use the real global starting price as the lowest room price; mock other prices conservatively. imageIndexes must reference IMAGE_URLS using zero-based indexes and never invent URLs. Use 3 room cards, up to 18 facilities and 3 attractions. Return one JSON object only.\n\nSOURCE_URL:${source.href}\nIMAGE_URLS:${JSON.stringify(images)}\n\nLISTING_TEXT:${listingText}\n\nREVIEWS_TEXT:${reviewsText}`;
    const groqResponse=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.GROQ_MODEL||'openai/gpt-oss-20b',messages:[{role:'system',content:'Extract facts and return valid JSON matching the requested shape. Keep all text concise.'},{role:'user',content:prompt}],temperature:.2,max_completion_tokens:2400,response_format:{type:'json_object'}})});
