...
export function ServiceShowcaseVisual({ type }: { type: string }) {
  if (type === 'website') {
    const projects = [
...
function HospitalityPackages({ locale }: { locale: 'ro' | 'en' | 'de' | 'no' }) {
  ...
  const basic = [
    l('Website modern și responsive', 'Modern responsive website', 'Moderne responsive Website', 'Moderne responsiv nettside'),
    ...
    l('SEO de bază și Google Business', 'Basic SEO and Google Business', 'Basis-SEO und Google Business', 'Grunnleggende SEO og Google Business'),
    l('Galerie, hartă și acțiuni WhatsApp', 'Gallery, map and WhatsApp actions', 'Galerie, Karte und WhatsApp-Aktionen', 'Galleri, kart og WhatsApp-handlinger'),
    l('Optimizare pentru mobil și viteză', 'Mobile and speed optimization', 'Mobil- und Geschwindigkeitsoptimierung', 'Mobil- og hastighetsoptimalisering'),
  ];
  const advanced = [
    l('Tot ce este inclus în pachetul Esențial', 'Everything in the Essential package', 'Alles aus dem Basispaket', 'Alt i basispakken'),
    l('Plăți online și rezervări directe', 'Online payments and direct bookings', 'Online-Zahlungen und Direktbuchungen', 'Nettbetaling og direktebestilling'),
    l('Consultanță Booking.com și Airbnb', 'Booking.com and Airbnb consulting', 'Booking.com- und Airbnb-Beratung', 'Rådgivning for Booking.com og Airbnb'),
    l('Channel manager și sincronizare calendar', 'Channel manager and calendar synchronization', 'Channel Manager und Kalendersynchronisierung', 'Channel manager og kalendersynkronisering'),
    l('SEO avansat, Google Travel și indexare', 'Advanced SEO, Google Travel and indexing', 'Erweiterte SEO, Google Travel und Indexierung', 'Avansert SEO, Google Travel og indeksering'),
    l('Promovare gratuită TikTok și YouTube Short', 'Free TikTok and YouTube Short promotion', 'Kostenlose TikTok- und YouTube-Short-Promotion', 'Gratis TikTok- og YouTube Short-promotering'),
    l('Analytics, conversii și raportare', 'Analytics, conversions and reporting', 'Analytics, Conversions und Berichte', 'Analytics, konverteringer og rapportering'),
  ];
  return <section className="py-24 lg:py-32 bg-slate-900/40 border-y border-white/5 relative overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-3xl mb-14"><span className="text-emerald-400 font-mono text-sm uppercase tracking-[.2em]">{l('Pachete hospitality', 'Hospitality packages', 'Hospitality-Pakete', 'Reiselivspakker')}</span><h2 className="text-4xl lg:text-5xl font-bold mt-4 mb-5">{l('Alege fundația potrivită proprietății tale.', 'Choose the right foundation for your property.', 'Wählen Sie die richtige Basis für Ihre Unterkunft.', 'Velg riktig grunnlag for eiendommen din.')}</h2><p className="text-lg text-slate-400">{l('Începe simplu sau construiește un canal complet pentru rezervări directe.', 'Start simple or build a complete direct-booking channel.', 'Starten Sie einfach oder bauen Sie einen vollständigen Direktbuchungskanal auf.', 'Start enkelt eller bygg en komplett kanal for direktebestilling.')}</p></div>
      <div className="grid lg:grid-cols-2 gap-7">
        <article className="glass rounded-3xl p-7 sm:p-10 flex flex-col"><div className="flex justify-between gap-5 mb-8"><div><p className="text-brand font-mono text-xs uppercase tracking-widest mb-2">{l('Esențial', 'Essential', 'Basis', 'Basis')}</p><h3 className="text-2xl font-bold">{l('Lansare Hospitality', 'Hospitality Launch', 'Hospitality Start', 'Hospitality lansering')}</h3></div><Globe2 className="h-9 w-9 text-brand"/></div><div className="mb-8"><strong className="text-4xl lg:text-5xl text-white">{launchPrice}</strong><p className="text-sm text-slate-500 mt-2">{l('Plată unică pentru website', 'One-time website setup', 'Einmalige Website-Erstellung', 'Engangspris for nettsiden')}</p></div><ul className="space-y-3 mb-9 flex-1">{basic.map(item=><li key={item} className="flex gap-3 text-sm text-slate-300"><CheckCircle2 className="h-5 w-5 text-brand shrink-0"/>{item}</li>)}</ul><div className="rounded-xl bg-white/5 border border-white/10 p-4 mb-5 text-sm"><b className="text-white">{monthlyPrice}</b> <span className="text-slate-400">{l('hosting și mentenanță', 'hosting and maintenance', 'Hosting und Wartung', 'hosting og vedlikehold')}</span></div><Link to="/contact" className="inline-flex justify-center items-center rounded-xl bg-white/10 hover:bg-white/15 border border-white/10 px-6 py-4 text-white font-bold transition">{l('Alege Esențial', 'Choose Essential', 'Basis wählen', 'Velg basis')}<ArrowRight className="ml-2 h-4 w-4"/></Link></article>
        <article className="rounded-3xl p-7 sm:p-10 flex flex-col bg-gradient-to-b from-emerald-500/15 to-white/[.03] border border-emerald-400/40 shadow-2xl shadow-emerald-500/10 relative"><span className="absolute top-0 right-8 -translate-y-1/2 rounded-full bg-emerald-500 text-slate-950 px-4 py-2 text-xs font-black uppercase">{l('Pentru creștere', 'For growth', 'Für Wachstum', 'For vekst')}</span><div className="flex justify-between gap-5 mb-8"><div><p className="text-emerald-400 font-mono text-xs uppercase tracking-widest mb-2">{l('Avansat', 'Advanced', 'Erweitert', 'Avansert')}</p><h3 className="text-2xl font-bold">{l('Rezervări Directe', 'Direct Booking Growth', 'Direktbuchungs-Wachstum', 'Vekst med direktebestilling')}</h3></div><CalendarCheck className="h-9 w-9 text-emerald-400"/></div><div className="mb-8"><strong className="text-4xl lg:text-5xl text-white">{l('Ofertă', 'Custom', 'Individuell', 'Tilpasset')}</strong><p className="text-sm text-slate-500 mt-2">{l('În funcție de proprietate și integrări', 'Based on property and integrations', 'Je nach Unterkunft und Integrationen', 'Basert på eiendom og integrasjoner')}</p></div><ul className="space-y-3 mb-9 flex-1">{advanced.map(item=><li key={item} className="flex gap-3 text-sm text-slate-300"><CheckCircle2 className="h-5 w-5 text-emerald-400 shrink-0"/>{item}</li>)}</ul><div className="grid sm:grid-cols-2 gap-3"><Link to="/demos/hospitality" className="inline-flex justify-center items-center rounded-xl bg-white/10 border border-white/10 px-5 py-4 text-white font-bold">{l('Vezi demo', 'View demo', 'Demo ansehen', 'Se demo')}</Link><Link to="/contact" className="inline-flex justify-center items-center rounded-xl bg-emerald-500 hover:bg-emerald-400 px-5 py-4 text-slate-950 font-black transition">{l('Solicită ofertă', 'Request a quote', 'Angebot anfragen', 'Be om tilbud')}</Link></div></article>
      </div>
      <p className="text-xs text-slate-600 mt-6">{l('Taxele platformelor, channel manager-ului și comisioanele terților nu sunt incluse.', 'Platform, channel-manager and third-party fees are not included.', 'Plattform-, Channel-Manager- und Drittanbietergebühren sind nicht enthalten.', 'Plattform-, channel-manager- og tredjepartsgebyrer er ikke inkludert.')}</p>
    </div>
  </section>;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid lg:grid-cols-[1.08fr_.92fr] gap-14 items-center relative">
        ...
        <motion.div initial={{opacity:0,scale:.94,y:20}} animate={{opacity:1,scale:1,y:0}} transition={{delay:.15}} className="relative">
          ...
          <div className="relative glass rounded-[2rem] p-3 shadow-2xl rotate-1 hover:rotate-0 transition duration-500">
            <div className="rounded-[1.4rem] bg-slate-950/90 border border-white/10 overflow-hidden min-h-[310px]"><ServiceShowcaseVisual type={c.visual}/></div>
          </div>
          <div className="absolute -bottom-5 -left-5 glass rounded-2xl px-5 py-4 shadow-xl flex items-center gap-3"><span className={`grid place-items-center h-10 w-10 rounded-xl bg-gradient-to-br ${meta.color}`}><Icon className="h-5 w-5 text-white"/></span><span className="text-sm text-white font-semibold">NodeStack<br/><small className="text-slate-500 font-normal">Built for growth</small></span></div>
        </motion.div>
      </div>
    </section>
