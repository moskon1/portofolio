...
export function ServiceShowcaseVisual({ type, projectHref }: { type: string; projectHref?: string }) {
  if (type === 'website') {
    const projects = [
      { name: 'RoVista', image: '/rovista-preview.png', href: 'https://rovista.ro/' },
    ...
    return <div className="p-4 sm:p-5 grid grid-cols-2 gap-3 h-[310px]">{projects.map((project) => <a key={project.name} href={projectHref || project.href} className="group relative overflow-hidden rounded-xl border border-white/10 bg-slate-900" target={!projectHref && project.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer"><img src={project.image} alt={project.name} referrerPolicy="no-referrer" className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-transparent to-transparent"/><span className="absolute bottom-3 left-3 right-3 text-xs sm:text-sm font-bold text-white">{project.name}</span></a>)}</div>;
  }
  if (type === 'hotel') return <a href={projectHref || '/demos/hospitality'} className="group relative block h-[310px] overflow-hidden"><img src="/rovista-preview.png" alt="RoVista hospitality website preview" className="h-full w-full object-cover transition duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/10 to-transparent"/><div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7"><span className="inline-flex rounded-full bg-emerald-400/15 border border-emerald-300/20 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-emerald-300 mb-3">Live demo</span><p className="text-xl font-bold text-white">Hospitality Demo</p><p className="text-xs text-slate-300 mt-1">Hotel & villa booking experience</p></div></a>;
...
function ServicePortfolio({ serviceKey, locale }: { serviceKey: 'websites' | 'seo' | 'web-applications'; locale: 'ro' | 'en' | 'de' | 'no' }) {
  const l = (ro: string, en: string, de: string, no: string) => localize(locale, { ro, en, de, no });
  const rovistaProject = { title: 'RoVista', category: l('Ghid turistic si aplicatie web','Travel guide and web application','Reisefuhrer und Webanwendung','Reiseguide og webapplikasjon'), image: '/rovista-preview.png', link: 'https://rovista.ro/' };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid lg:grid-cols-[1.08fr_.92fr] gap-14 items-center relative">
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}}>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/contact" className="inline-flex items-center justify-center bg-brand hover:bg-brand-dark text-white px-7 py-4 rounded-xl font-bold transition shadow-xl shadow-brand/20">{localize(locale,{ro:'Solicită o ofertă',en:'Request a quote',de:'Angebot anfragen',no:'Be om tilbud'})}<ArrowRight className="ml-2 h-5 w-5"/></Link>
            {serviceKey === 'hospitality' && <Link href="/demos/hospitality" className="inline-flex items-center justify-center bg-white/5 hover:bg-white/10 border border-white/10 text-white px-7 py-4 rounded-xl font-bold transition">{localize(locale,{ro:'Vezi demo live',en:'View live demo',de:'Live-Demo ansehen',no:'Se live-demo'})}</Link>}
          </div>
        </motion.div>
