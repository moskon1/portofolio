      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link to="/" className={cn("text-sm font-medium transition-colors hover:text-brand",location.pathname==='/'?'text-brand':'text-slate-400')}>{localize(locale,{ro:'Acasă',en:'Home',de:'Startseite',no:'Hjem'})}</Link>
            <div className="relative group py-7">
              <button className={cn("flex items-center gap-1.5 text-sm font-medium transition-colors group-hover:text-brand", location.pathname.startsWith('/services/') ? 'text-brand' : 'text-slate-400')}>{servicesLabel}<ChevronDown className="h-4 w-4 transition-transform group-hover:rotate-180"/></button>
              <div className="invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 absolute top-[68px] left-1/2 -translate-x-1/2 w-80 rounded-2xl border border-white/10 bg-slate-950/95 backdrop-blur-xl p-2 shadow-2xl transition-all duration-200">
                {serviceLinks.map(({name,href,icon:Icon})=><Link key={href} to={href} className={cn("flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition",location.pathname===href?'bg-brand/10 text-brand':'text-slate-300 hover:bg-white/5 hover:text-white')}><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/5"><Icon className="h-4 w-4"/></span>{name}</Link>)}
              </div>
            </div>
            {navLinks.map((link) => (
              <Link
      ...
      {isOpen && (
        <div className="md:hidden bg-slate-900 border-b border-white/5 animate-in slide-in-from-top duration-300">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link to="/" onClick={()=>setIsOpen(false)} className={cn("block px-3 py-2 rounded-md text-base font-medium",location.pathname==='/'?'text-brand bg-brand/10':'text-slate-400 hover:text-brand hover:bg-white/5')}>{localize(locale,{ro:'Acasă',en:'Home',de:'Startseite',no:'Hjem'})}</Link>
            <p className="px-3 pt-2 pb-1 text-xs font-bold uppercase tracking-widest text-slate-600">{servicesLabel}</p>
            {serviceLinks.map(({name,href,icon:Icon})=><Link key={href} to={href} onClick={()=>setIsOpen(false)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium",location.pathname===href?'text-brand bg-brand/10':'text-slate-400 hover:text-brand hover:bg-white/5')}><Icon className="h-4 w-4"/>{name}</Link>)}
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.href}
